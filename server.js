...
async function generateQuestion(value, prompt) {
    ...
    return data.message.trim();
        ...
        const data = await response.json()
        console.log(data);
        return 'Internal Server Error. Please try again later. If the situation persists please contact the developers.';
    }
}
function getMessage(msg, prompt) {
    switch(msg){
        ...
        case '5':
                return `Write 5 challenging A Level Maths questions based on the topic of ${prompt}. Write these questions in MathJax format, wrapping any equations under the inline syntax.`;
    }
}
app.listen(port, () => {
